...
describe('generateFindingTests', () => {
  it('generates config + fixture + specs for each finding', () => {
    ...
    expect(paths).toContain('playwright-tests/playwright.config.ts');
    expect(paths).toContain('playwright-tests/fixtures/findings.ts');
    expect(paths).toContain('playwright-tests/fixtures/auth.ts');
  ...
  it('spec file is valid Playwright code', () => {
    ...
    const spec = result.files.find(f => f.path === 'playwright-tests/attack-f-1.spec.ts')!;
