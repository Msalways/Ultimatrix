...
export class BrowserSessionManager {
  ...
  async startTrace(sessionId: string): Promise<string> {
    ...
    state.context.on('response', (res) => {
      ...
      if (entry) {
        ...
        // Capture response body (truncated) for reflection/param discovery.
        // Best-effort — don't let a body read failure break tracing.
        res.text().then(
          (t) => { if (entry) entry.responseBody = t.slice(0, 2000); },
  ...
  /**
   * Return the number of trace entries recorded so far for a session.
   */
  getTraceLength(sessionId: string): number {
    const state = this.sessions.get(sessionId);
    return state?.trace.length ?? 0;
  }

  /**
   * Phase 3A: tag the most recent trace entries. Used by the spider's
   * form submission handler to mark entries generated by a form submit.
   */
  tagRecentTraceEntries(sessionId: string, tags: string[], count: number): void {
    const state = this.sessions.get(sessionId);
    if (!state || !state.trace.length) return;
    const start = Math.max(0, state.trace.length - count);
    for (let i = start; i < state.trace.length; i++) {
      state.trace[i].tags = [...(state.trace[i].tags || []), ...tags];
    }
  }
