  ...
  private async exploreFormsOnPage(
    ...
    for (const form of forms) {
      ...
      try {
        ...
        // Phase 3A: record the trace length before the submission so
        // we can tag any new trace entries as generated by this form.
        const traceBefore = this.manager.getTraceLength(this.sessionId);
        ...
        // Phase 3A: tag the entries generated by this form submission
        const traceAfter = this.manager.getTraceLength(this.sessionId);
        const newEntryCount = traceAfter - traceBefore;
        if (newEntryCount > 0) {
          this.manager.tagRecentTraceEntries(this.sessionId, [`form-submit:${form.action || absAction}`], newEntryCount);
        }
