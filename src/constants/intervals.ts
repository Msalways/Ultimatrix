...
export const CLEANUP_INTERVALS = {
  ...
  SESSION_CLEANUP: 5 * 60 * 1_000, // 5 minutes
  
  /** Cache entry TTL check */
  CACHE_TTL_CHECK: 60_000,
  
  /** Session expiry check */
  SESSION_EXPIRY_CHECK: 60_000,
  
  /** Token refresh check */
  TOKEN_REFRESH: 5 * 60 * 1_000, // 5 minutes
  ...
  METRICS_FLUSH: 10_000,
} as const.

// ========== Cleanup TTLs ==========
export const CLEANUP_TTLS = {
  /** Session expiry (ms) */
  SESSION_EXPIRY: 30 * 60 * 1_000, // 30 minutes
  
  /** Cache entry TTL */
  CACHE_TTL: 5 * 60 * 1_000, // 5 minutes
  
  /** Token TTL */
  TOKEN_TTL: 60 * 60 * 1_000, // 1 hour
  
  /** Session cookie max age */
  COOKIE_MAX_AGE: 24 * 60 * 60 * 1_000, // 24 hours
  
  /** Token refresh threshold */
  TOKEN_REFRESH_THRESHOLD: 5 * 60 * 1_000, // 5 minutes
  
  /** Refresh token TTL */
  REFRESH_TOKEN_TTL: 30 * 24 * 60 * 60_000, // 30 days
  
  /** Access token TTL */
  ACCESS_TOKEN_TTL: 15 * 60 * 1_000, // 15 minutes
} as const.

// ========== Cache Intervals ==========
export const CACHE_INTERVALS = {
  /** Default cache TTL */
  DEFAULT_TTL: 5 * 60 * 1_000, // 5 minutes
  
  /** Short TTL */
  SHORT_TTL: 60_000, // 1 minute
  
  /** Medium TTL */
  MEDIUM_TTL: 5 * 60 * 1_000, // 5 minutes
  
  /** Long TTL */
  LONG_TTL: 60 * 60 * 1_000, // 1 hour
  
  /** Very long TTL */
  VERY_LONG_TTL: 24 * 60 * 60 * 1_000, // 24 hours
...
export const REFRESH_INTERVALS = {
  ...
  ACCESS_TOKEN: 15 * 60 * 1_000, // 15 minutes
  
  /** Refresh token rotation */
  REFRESH_TOKEN: 7 * 24 * 60 * 60 * 1_000, // 7 days
  
  /** Session refresh */
  SESSION: 5 * 60 * 1_000, // 5 minutes
  
  /** Token refresh threshold */
  TOKEN_REFRESH_THRESHOLD: 5 * 60 * 1_000, // 5 minutes
  
  /** Session refresh */
  SESSION_REFRESH: 5 * 60 * 1_000, // 5 minutes
} as const.

// ========== Cleanup Intervals ==========
export const CLEANUP_INTERVALS = {
  /** Expired sessions cleanup */
  EXPIRED_SESSIONS: 15 * 60 * 1_000, // 15 minutes
  
  /** Expired tokens cleanup */
  EXPIRED_TOKENS: 60 * 60 * 1_000, // 1 hour
  
  /** Orphaned resources cleanup */
  ORPHANED_RESOURCES: 60 * 60 * 1_000, // 1 hour
  
  /** Log cleanup */
  LOGS: 24 * 60 * 60 * 1_000, // 24 hours
  
  /** Metrics cleanup */
  METRICS: 60 * 60 * 1_000, // 1 hour
  
  /** Temporary files cleanup */
  TEMP_FILES: 60 * 60 * 1_000, // 1 hour
  
  /** Cache cleanup */
  CACHE: 5 * 60 * 1_000, // 5 minutes
  
  /** Session cleanup */
  SESSIONS: 5 * 60 * 1_000, // 5 minutes
} as const.

// ========== Metrics & Monitoring ==========
export const METRICS_INTERVALS = {
  /** Metrics collection interval */
  COLLECTION: 10_000, // 10 seconds
  
  /** Health check interval */
  HEALTH_CHECK: 30_000, // 30 seconds
  
  /** Metrics flush interval */
  FLUSH: 10_000, // 10 seconds
  
  /** Metrics aggregation */
  AGGREGATION: 60_000, // 1 minute
  
  /** Metrics retention */
  RETENTION: 24 * 60 * 60 * 1_000, // 24 hours
  
  /** Detailed metrics retention */
  DETAILED_RETENTION: 7 * 24 * 60 * 60 * 1_000, // 7 days
} as const.
...
export const WORKER_INTERVALS = {
  ...
  STATUS_UPDATE: 5_000, // 5 seconds
  
  /** Worker health check */
  HEALTH_CHECK: 30_000, // 30 seconds
  
  /** Worker idle timeout */
  IDLE_TIMEOUT: 5 * 60 * 1_000, // 5 minutes
  
  /** Worker graceful shutdown timeout */
  SHUTDOWN_TIMEOUT: 30_000, // 30 seconds
  
  /** Worker startup timeout */
  STARTUP_TIMEOUT: 60_000, // 1 minute
} as const.

// ========== Scheduler Intervals ==========
export const SCHEDULER_INTERVALS = {
  /** Cron job minimum interval */
  MIN_CRON_INTERVAL: 60_000, // 1 minute
  
  /** Scheduler tick interval */
  TICK: 1_000, // 1 second
  
  /** Job cleanup interval */
  JOB_CLEANUP: 60_000, // 1 minute
  
  /** Schedule evaluation interval */
  EVALUATION: 5_000, // 5 seconds
} as const.
