...
import { getGlobalOastStore } from './store'
...
describe('OAST server', () => {
  ...
  afterEach(async () => {
    const { stopOastServer } = await import('./server')
    await stopOastServer()
  })

  it('startOastServer returns a port', async () => {
    const { startOastServer, stopOastServer } = await import('./server')
  ...
  it('getOastUrl returns correct format after start', async () => {
    const { startOastServer, getOastUrl, stopOastServer } = await import('./server')
