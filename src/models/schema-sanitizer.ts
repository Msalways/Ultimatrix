...
import { standardSchemaToJSONSchema, type StandardSchemaV1 } from '@mastra/schema-compat/schema'
...
function stripKeywords(obj: unknown, keywords: string[]): unknown {
  ...
  if (Array.isArray(obj)) return obj.map(item => stripKeywords(item, keywords))
...
export function createSanitizedInputSchema(
  ...
  if (level === 'none') {
    return {
      ...
      '~standard': (standardSchema as Record<string, unknown>)['~standard'],
    }
  }

  // Sanitize for strict provider
  const unsupported = level === 'strict' ? KEYWORDS_STRICT : KEYWORDS_MODERATE
  const sanitized = stripKeywords(rawJsonSchema, unsupported) as Record<string, unknown>
 ...
 */
export function applySchemaCompat<T extends { inputSchema?: unknown; id?: string }>(
  ...
  for (const [key, tool] of Object.entries(tools)) {
    if (tool.inputSchema && typeof tool.inputSchema === 'object' && '~standard' in (tool.inputSchema as object)) {
      const sanitized = createSanitizedInputSchema(tool.inputSchema as StandardSchemaV1, provider)
...
function sanitizeToolParamsInArray(tools: unknown[], keywords: string[]): unknown[] {
    ...
    if (tool && typeof tool === 'object') {
      ...
      if (t.function && typeof t.function === 'object') {
        ...
        if (fn.parameters && typeof fn.parameters === 'object') {
          return { ...t, function: { ...fn, parameters: stripKeywords(fn.parameters, keywords) } }
        }
      }
...
function auditKeywords(obj: unknown, keywords: string[], path: string, audit: string[]): void {
  if (obj === null || obj === undefined || typeof obj !== 'object') return
  if (Array.isArray(obj)) {
    obj.forEach((item, i) => auditKeywords(item, keywords, `${path}[${i}]`, audit))
    return
  }
  for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
