...
### 5.1 Directly required by this plan (all DONE — verified in prior sessions)
- [x] **Live HAR capture fix (L1)** — CDP-native capture via `src/session/cdp-network-capture.ts`,
  HAR owned by `har-parser.ts`, full ExtraInfo event set, origin tagging. (§8)
- [x] **Relation graph edges (L2)** — `REINGESTS` / `ORDERED_BEFORE` / `PROVES` edges + `EndpointNode.endpointKey`.
- [x] **Relational query tool (L3)** — `queryRelations` in `src/graph/relation-tools.ts`.
- [x] **`hypothesis-engine` regex removal (L4)** — relation-native; `looksLikeId`/secret shapes retained.
- [x] **`ExploitProof` node (L2/L7)** — `NodeType.EXPLOIT_PROOF` + `ExploitProofNode`; emitted by primitives (`runPrimitiveById` seam) and `writeFinding` (`exploitProof` arg + `PROVES` edge).
