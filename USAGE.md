...
The test will fail when a previously confirmed vulnerability is fixed, and fail again if it regresses. Free CI for security.

Every primitive that declares `toPlaywrightStep` metadata (e.g. `httpRequest`, `evaluateRendered`, `spiderCrawl`) gets auto-recorded by the recording plugin — no LLM prompt engineering needed to capture test steps.
...
### Add a plugin with lifecycle hooks
...
getGlobalRegistry().registerPlugin({
  ...
  primitives: {},
  hooks: {
    beforePrimitive: async (name, args, ctx) => { /* log, modify, veto */ },
    afterPrimitive: async (name, args, result, ctx) => { /* record, alert */ },
    onFinding: async (finding) => { /* notify, file bug */ },
  },
});
```
