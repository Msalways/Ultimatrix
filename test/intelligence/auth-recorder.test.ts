...
describe('auth-recorder', () => {
  ...
  describe('detectLoginForm', () => {
    it('returns null when no password field in snapshot', async () => {
      const { detectLoginForm } = await import('../../src/intelligence/auth-recorder')
  ...
  describe('createAuthFlow', () => {
    it('creates an auth flow and saves', async () => {
      const { createAuthFlow } = await import('../../src/intelligence/auth-recorder')
  ...
  describe('getReusableAuthFlow', () => {
    it('returns first reusable flow', async () => {
      const { getReusableAuthFlow } = await import('../../src/intelligence/auth-recorder')
      const flows = [
  ...
  describe('replayAuthFlow', () => {
    it('replays each step using executeStep', async () => {
      const { replayAuthFlow } = await import('../../src/intelligence/auth-recorder')
      const steps = [
        { action: 'goto', url: '/login' },
    ...
    it('returns null when no logout in snapshot', async () => {
      const { detectLogoutFlow } = await import('../../src/intelligence/auth-recorder')
  ...
  describe('detectTokenRefreshFlow', () => {
    it('returns null when no refresh token in snapshot', async () => {
      const { detectTokenRefreshFlow } = await import('../../src/intelligence/auth-recorder')
  ...
  describe('isAuthExpired', () => {
    it('returns true for 401', async () => {
      const { isAuthExpired } = await import('../../src/intelligence/auth-recorder')
